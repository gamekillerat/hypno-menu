...
void* Hooking::mon;
char* Hooking::loading_text = "Loading GTA Online with HypnoV2";
...
const char* __cdecl hkGetLabelText(void *this_, const char *label)
{
	...
	{
		return "Loading HypnoV2";
	}

	if (std::strcmp(label, "HUD_QUITTING") == 0)
	{
		return "Leaving with HypnoV2";
	}

	if (std::strcmp(label, "PM_QUIT_MP") == 0)
	{
		return "Exiting GTA Online with HypnoV2";
	}

	if (std::strcmp(label, "PM_ENTER_MP") == 0)
	{
		return "[Join with HypnoV2] -> Play GTA Online";
	}

	if (std::strcmp(label, "PM_EXIT_GAME") == 0)
	{
		return "Quit GTA with HypnoV2";
	}

	if (std::strcmp(label, "PM_GO") == 0)
	{
		return "[Join with HypnoV2] -> Go (Freemode)";
	}

	if (std::strcmp(label, "HUD_MPREENTER") == 0)
	{
		return "Joining a new session because your old one was trash. (w/ HypnoV2 ofc)";
	}

	if (std::strcmp(label, "HUD_ROSBANNED") == 0)
	{
		return "Oops, you've been banned permanantly from GTA Online! Please fill out the ban report from in our Discord server. Continue to GTA V with HypnoV2?";
	}

	if (std::strcmp(label, "HUD_ROSBANPERM") == 0)
	{
		return "Oops, you've been banned permanantly from GTA Online! Please fill out the ban report from in our Discord server. Continue to GTA V with HypnoV2?";
	}

	if (std::strcmp(label, "HUD_ROSBANX") == 0)
	{
		return "You got banned until ~a~.~n~In addition, your Grand Theft Auto Online character(s) will be reset.";
	}

	if (std::strcmp(label, "PM_FRIEND_FM") == 0)
	{
		return "[Join with HypnoV2] -> Closed Friend Session";
	}

	if (std::strcmp(label, "PM_GOTO_STORE") == 0)
	{
		return "Purchase Shark Card (like a loser)";
	}

	if (std::strcmp(label, "PM_INF_EXIT") == 0)
	{
		return "Quit to desktop and shut down HypnoV2?";
	}

	if (std::strcmp(label, "HUD_UPDATEBAIL") == 0)
	{
		return "Oh, there seems to be a new update! So, therefore, you have been removed from the session. Please wait approx 2 days for HypnoV2 to update to this latest patch!";
	}

	if (std::strcmp(label, "HUD_BANFEED") == 0)
	{
		return "Able to have fun with HypnoV2 on GTA Online again date ~1~/~1~/~1~.";
	}

	if (std::strcmp(label, "PM_CRESES") == 0)
	{
		return "[Join with HypnoV2] -> Join Crew Members";
	}

	if (std::strcmp(label, "PM_INF_PGOB") == 0)
	{
		return "Cut to the chase and head straight into the world of GTA Online. [With HypnoV2 of course, why not?]";
	}

	if (std::strcmp(label, "PM_INF_PGOT") == 0)
	{
		return "Online w/ Hypno!";
	}

	if (std::strcmp(label, "PM_INVO_FM") == 0)
	{
		return "[Join with HypnoV2] -> Invite Only Session";
	}

	if (std::strcmp(label, "LOADING_SPLAYER_L") == 0)
	{
		return "Loading Story Mode. HypnoV2 also Injected.";
	}

	if (std::strcmp(label, "PM_NCREW_FM") == 0)
	{
		return "[Join with HypnoV2] -> Crew Session";
	}
	
	if (std::strcmp(label, "PM_CREW_FM") == 0)
	{
		return "[Join with HypnoV2] -> Closed Crew Session";
	}

	if (std::strcmp(label, "PM_SOLO_FM") == 0)
	{
		return "[Join with HypnoV2] -> Solo Session";
	}

	if (std::strcmp(label, "PM_CRESES") == 0)
	{
		return "[Join with HypnoV2] -> Crew Members";
	}

	if (std::strcmp(label, "PM_FRESES") == 0)
	{
		return "[Join with HypnoV2] -> Friends";
	}

	if (std::strcmp(label, "PCARD_SP") == 0)
	{
		return "Grand Theft Auto V | HypnoV2";
	}

	if (std::strcmp(label, "PM_PAUSE_HDR") == 0)
	{
		return "Grand Theft Auto V | HypnoV2";
	}
...
bool __cdecl HK_STAT_SET_INT(Hash statName, int value, bool save)
{
	...
	case 0x9F79BA0B: //MPPLY_EXPLOITS
	{
		...
		Log::Msg("Event | Blocked bad stat: 0x%08X Value: %i", statName, value);
		return false;
	}
	}
	...
	/*for (int i = 0; i < 656; i++) {
		...
		if (strcmp(message, base_cmd.c_str()) == 0) {
			...
			if (STREAMING::IS_MODEL_IN_CDIMAGE(model) && STREAMING::IS_MODEL_A_VEHICLE(model)) {
				...
				Vehicle veh = Hooking::create_vehicle(model, coords.x, coords.y, coords.z, 0, 1, 1);
...
bool Hooking::HookNatives()
{
	...
	if (enable_coloading) {
		...
		if ((status != MH_OK && status != MH_ERROR_ALREADY_CREATED) || MH_EnableHook(Hooking::is_DLC_present) != MH_OK)
			return false;
		Hooking::m_hooks.push_back(Hooking::is_DLC_present);
		Log::Hooking("Main Hook: IDP");

		/*status = MH_CreateHook(Hooking::is_entity_upsidedown, HK_IS_ENTITY_UPSIDEDOWN, (void**)&OG_IS_ENTITY_UPSIDEDOWN);
		if ((status != MH_OK && status != MH_ERROR_ALREADY_CREATED) || MH_EnableHook(Hooking::is_entity_upsidedown) != MH_OK)
			return false;
		Hooking::m_hooks.push_back(Hooking::is_entity_upsidedown);
		Log::Msg("Hooking | Main: DCE");*/
...
void __stdcall ScriptFunction(LPVOID lpParameter)
{
	...
	{
		Log::Fatal("Please ensure you have the directory 'C:/Hypno/'.");
	}
}
...
bool Hooking::GED(int eventGroup, int eventIndex, uint64_t* argStruct, int argStructSize)
{
	...
	if (result && blockall) {
		...
		if (PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(argStruct[1]) != PLAYER::PLAYER_PED_ID() || PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(argStruct[1]) != PLAYER::PLAYER_ID()) {
			...
			if (notify) {
				...
				if (logger) {
					Log::Msg("===============================");
					Log::Msg("Event | Blocked event sent by %s", name);
					Log::Msg("Event | Script event group: %i", eventGroup);
					Log::Msg("Event | Script event index: %i", eventIndex);
					Log::Msg("Event | Script event argcount: %i", argStructSize);
					for (std::uint32_t i = 0; i < argStructSize; ++i) {
						Log::Msg("Event | Script event args[%u] : %u", i, argStruct[i]);
					}
				}
			}
		}
	...
	/*else if (result && smart_protex)
	{
		...
		if (pname != invalid[0] && pname != invalid[1] && pname != invalid[2] && pname != PLAYER::GET_PLAYER_NAME(PLAYER::PLAYER_ID())) {
			Features::write2("C:/Hypno/Metadata/Configuration/LogScriptEvents.txt", std::to_string(argStruct[0]));
			Log::Msg("Added event to blacklist: %u", argStruct[0]);
		}
	}*/

	else if (result && smart_pro) {
		if (Features::CheckWord("C:/Hypno/Metadata/Configuration/LogScriptEvents.txt", (char*)std::to_string(argStruct[0]).c_str())) {
			char* name = PLAYER::GET_PLAYER_NAME(argStruct[1]);
			char buf[30];
			char id[30];
			char sender[30];
...
)
{
	...
	while (true)
	{
		if (ptr == nullptr) {
			//failPat(name);		
			if (enable_coloading) {
				Log::HookingSkip("Hooking | Skipped %s pattern", name);
			}
			else {
				failPat(name);
			}
...
)
{
	...
	if (ptr == nullptr) {
		//failPat(name);
		if (enable_coloading) {
			Log::HookingSkip("Skipped %s pattern", name);
		}
		else {
			failPat(name);
		}
...
void HookMsg(std::string txt, bool hooked = false) {
	hooked ? Log::Hooking("Pattern > %s", txt) : Log::Hooking("Pattern > %s", txt);
}
...
void Hooking::FindPatterns()
{
	...
	HookMsg("SNITS");
	Log::Msg("Load > VMT");
	Log::Msg("Hooked Successfully");
	Log::Msg("Base Address: 0x00007FF689F30000");
	Log::Msg("Initialize Hooks > MH 1/2");
	Log::Msg("Initialize Hooks > MH 2/2");
	Log::Msg("Hooked > GNE");
	Log::Msg("Hooked > CCH");
	Log::Msg("Hooked > GED");
	Log::Msg("Hooked > GSH");
	Log::Msg("Discord RP Loaded...");
	Log::Msg("Hooked > IDP");
	Log::Msg("Hooked > SNITS");
	Log::Msg("Hooked > NSPI");
	Log::Msg("Hooked > NSIPVTK");
	Log::Msg("Hooked > JH");
	Log::Msg("Hooked > BH");
	Log::Msg("Hooked > HKB");
	Log::Msg("Hooked > EH");
...
{
	Log::Msg("Cleanup | Cleaning up hooks");
	ownedExplosionBypass(false);
	Log::Msg("Cleanup | Disabled OE bypass");
	...
	Log::Msg("Cleanup | Disabled all hooked natives");
	MH_Uninitialize();
	Log::Msg("Cleanup | Uninitialize finished");
}
