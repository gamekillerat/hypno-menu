...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: Hypno");
		Menu::addText("[" + std::to_string(current_tm.tm_hour) + ":" + std::to_string(current_tm.tm_min) + ":" + std::to_string(current_tm.tm_sec) + "] " + "INIT: Hypno", true);
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "HypnoMenuV2.log");
...
void Log::Msg(const char* fmt, ...) {
	...
	localtime_s(&current_tm, &current_time);
	sprintf_s(szTimestamp, "[%02d:%02d:%02d][Info]%%s\n", current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
...
void Log::Hooking(const char* fmt, ...) {
	...
	localtime_s(&current_tm, &current_time);
	sprintf_s(szTimestamp, "[%02d:%02d:%02d][Hooking]%%s\n", current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
...
void Log::HookingSkip(const char* fmt, ...) {
...
void Log::Game(const char* fmt, ...) {
	...
	localtime_s(&current_tm, &current_time);
	sprintf_s(szTimestamp, "[%02d:%02d:%02d][Game]%%s\n", current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
