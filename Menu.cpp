...
static char* CasinoHeist[] = {
	"ASBO", "KANJO", "MINITANK", "YOSEMITE2", "CARACARA2", "EVERON", "REBLA", "KOMODA", "IMORGON", "VSTR"
};
...
static char* dlcnames[] = {
	...
	"After Hours", "Arena War",
	"The Diamond Casino Heist", 
	"Los Santos Summer Special"
};

static char* dlcnames2[] = {
	"~r~[1.06] ~w~Beach Bum", "~r~[1.10] ~w~Valentines Massacre", "~r~[1.11] ~w~Business", "~r~[1.13] ~w~High Life", "~r~[1.14] ~w~I'm not a Hipster",
	"~r~[1.15] ~w~Independence Day", "~r~[1.16] ~w~Flight School", "~r~[1.17] ~w~Last Team Standing", "~r~[1.19] ~w~Festive 2014",
	"~r~[1.21] ~w~Heists", "~r~[1.27] ~w~Ill-gotton-gains", "~r~[1.30] ~w~Lowrider", "~r~[1.30] ~w~Halloween 2015", "~r~[1.31] ~w~Executives",
	"~r~[1.32] ~w~Festive 2015", "~r~[1.32] ~w~Drop zone", "~r~[1.32] ~w~Be My Valentine", "~r~[1.33] ~w~Custom Classics", "~r~[1.34] ~w~Finance and Felony",
	"~r~[1.35] ~w~Cunning Stunts", "~r~[1.36] ~w~Bikers", "~r~[1.37] ~w~Import / Export", "~r~[1.38] ~w~Special Vehicle Circuit", "~r~[1.40] ~w~Gunrunning",
	"~r~[1.41] ~w~Smugglers Run", "~r~[1.42] ~w~Doomsday", "~r~[1.43] ~w~Southern San Andreas", "~r~[1.44] ~w~After Hours", "~r~[1.46] ~w~Arena War", "~r~[1.48] [1.50] ~w~The Diamond Casino Heist", "~r~[1.51] [1.52] ~w~Los Santos Summer Special"
};
...
public:
	...
	void text_component(const char* text)
	{
	...
	void add(const std::string& text, char* button)
	{
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(handle, "SET_DATA_SLOT");
		GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(bc);
...
/*bool scrollbarbigboi = true;
bool scrollbarbigboi2 = true;*//*
std::string PlayerNameSelected2((PLAYER::GET_PLAYER_NAME(Features::Online::selectedPlayer)));
std::string WelcomeNotification = "Welcome ~r~" + PlayerNameSelected2 + " ~w~to Hypno ~g~VIP~w~!";*/

void notifybanner(const std::string& text)
{
	int32_t handle = 0;
	...
	GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
	GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(handle, "SHOW_SHARD_RANKUP_MP_MESSAGE");
	GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING("~r~Hypno");
	GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING(text.c_str());
	GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(5);
	GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
}
bool overtheme = false;

void main() {

	if (Cursor::cursorOn == false) { if (GetAsyncKeyState(VK_F12)) { Cursor::cursorOn = true; } }
	else if (Cursor::cursorOn == true) { if (GetAsyncKeyState(VK_F12)) { Cursor::cursorOn = false; } }

	if (firsttime) {
		HypnoScript::HudColourChange();
		player_name = SOCIALCLUB::_SC_GET_NICKNAME();
		Features::loadFromINI();
		Log::Game("Executed main function");
		menyooX = INI::GetFloat("C:/Hypno/Metadata/Configuration/config.ini", "theme", "menux") * 100;
		smooth_scroll_kek = INI::GetFloat("C:/Hypno/Metadata/Configuration/config.ini", "delay", "smooth") * 1000;
		display_when_open = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "close");
		ipdisplay = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "d_ip");
		Menu::Settings::selfInfo = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "d_info");
		if (GetModuleHandleA("steam_api64.dll"))
		{
			Log::Game("Version : Steam");
		}
		else if (GetModuleHandleA("EOSSDK-Win64-Shipping.dll"))
		{
			Log::Game("Version : Epic Games");
		}
		else
		{
			Log::Game("Version : Rockstar Games");
		}
		firstnoti = true;
		bannernoti = true;
		overtheme = false;
	...
	Thread::OnTick();
	std::string PlayerNameSelected((PLAYER::GET_PLAYER_NAME(Features::Online::selectedPlayer)));
	std::string TheBreakSayingPlayer = "Options for: ~g~" + PlayerNameSelected;
	std::string InstrucitonsWithNickname = "SC Username: " + PlayerNameSelected;

	if (firstnoti) {
		YTD();
		YTD2();
		Features::injectnotify("Welcome to Hypno!");
		Features::notify("Press * or F5.");
		firstnoti = false;
	}
	bool widthfix = true;
	...
	case mainmenu:
	{
		...
		std::string maintextvers(menu_version_);
		std::string maintext = "Hypno"/* + maintextvers*/;
		if (overtheme)
		{
			Menu::Break("Over");
		}
		...
		{
			Menu::Break(maintext.c_str());
		}
		Menu::MenuOption("Player", self);                       //Correct//
		Menu::MenuOption("Network", aioplist);                  //Correct//
		Menu::MenuOption("Weapons", weapons);                   //Correct//
		Menu::MenuOption("Vehicle Options", vehicles);          //Correct//
		Menu::MenuOption("Vehicle Spawner", spawner);           //Correct//
		Menu::MenuOption("Teleport", teleport);                 //Correct//
		Menu::MenuOption("Object Spawner", spwnerr);            //Correct//
		Menu::MenuOption("Misc", misc);                         //Correct// 
		if (Menu::Option("~c~ScriptHookV")) { Features::notify("~r~Unstable~w~, currently disabled."); }  //          Let off           //
		Menu::MenuOption("Crafting Laboratory", world);                     //         Fix please         //         Fix please         //         Fix please         //         Fix please         //
		Menu::MenuOption("Protections", protex);                //Correct//
		Menu::MenuOption("Recovery Options", recoy);            //Correct//
		Menu::MenuOption("Settings", settings);                 //Correct//
		if (!overtheme) { Menu::Option("~r~Beta ~w~0.5.5"); }
		Menu::Settings::menuWidth = (float)widthkek2 / 100;
		Menu::Settings::menuWidth = (float)widthkek / 100;
		Menu::Settings::scrollbarWidthIndex = (float)(Menu::Settings::menuWidth / 2) + 0.015f;
		if (Menu::Settings::currentOption == 1) { Menu::InfoText("Player Options"); }
		if (Menu::Settings::currentOption == 2) { Menu::InfoText("Network Options, like playerlist and such."); }
		if (Menu::Settings::currentOption == 3) { Menu::InfoText("Weapon Options (Rapid Fire, Unlim Ammo, etc.)"); }
		if (Menu::Settings::currentOption == 4) { Menu::InfoText("Edit vehicles and customize them"); }
		if (Menu::Settings::currentOption == 5) { Menu::InfoText("Spawn your vehicle of choice"); }
		if (Menu::Settings::currentOption == 6) { Menu::InfoText("Teleport to North Yankton!"); }
		if (Menu::Settings::currentOption == 7) { Menu::InfoText("Object Spawner for Peds / Objects"); }
		if (Menu::Settings::currentOption == 8) { Menu::InfoText("Micellaneous options"); }
		if (Menu::Settings::currentOption == 9) { Menu::InfoText("SHV Loader, ~r~doesn't work."); }
		if (Menu::Settings::currentOption == 10) { Menu::InfoText("Crafting Lab (Co-ord stuff)"); }
		if (Menu::Settings::currentOption == 11) { Menu::InfoText("Protect yourself from other modders"); }
		if (Menu::Settings::currentOption == 12) { Menu::InfoText("Recovery, ~r~may be risky~w~."); }
		if (Menu::Settings::currentOption == 13) { Menu::InfoText("Settings (Width, Color, etc.)"); }
		if (Menu::Settings::currentOption == 14) { Menu::InfoText("Version Beta 0.5.5"); }
	}
	break;
#pragma region protection
	case protex:
	{
		...
		Menu::Toggle("Kick Protection", Features::antiKick, [] { Features::AntiKick(Features::antiKick); });
		Menu::Toggle("Freeze Protection", Features::antiFreeze, [] { Features::AntiFreeze(Features::antiFreeze); });
		...
		Menu::Toggle("Stat Protection", Features::antiStat, [] { Features::AntiStat(Features::antiStat); });
		...
		Menu::Toggle("Rockstar Dev Protection", Features::devDisabled);
		Menu::Toggle("Vehicle Freeze Protection", Features::antivf);
		Menu::Toggle("Crash Protection", Features::anticr);
...
#pragma endregion
	case credits:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Credits");
		if (Menu::Option("iAmSpace"))
		{
			system("start https://discord.com/invite/sXQKGb2");
			system("start https://www.youtube.com/channel/UC96NYOG1SGlYPqVIM11LaAQ");
		}
		if (Menu::Option("Trixxy"))
		{
			system("start https://discord.com/invite/3tqpxfC");
			system("start https://www.youtube.com/channel/UCITMJgEcLIZJs7cLxJIzvOg");
		}
		if (Menu::Option("Darks"))
		{
			system("start https://discord.com/invite/JSk33Tg");
			system("start https://www.youtube.com/channel/UC-Slj9vY4vEE9aRGiWDt8LQ");
		}
		if (Menu::Option("Senpai | Lucid"))
		{
			system("start https://www.youtube.com/channel/UCzHyNeXmveEXeoRer7LtcZg");
		}
		if (Menu::Option("aki"))
		{
			system("start https://discord.com/invite/vxfF2xs");
			system("start https://www.youtube.com/channel/UCp3_qCMxeGQ9tqBP2kobWow");
		}
		if (Menu::Option("Crabi"))
		{
			system("start https://discord.com/invite/");
			system("start https://www.youtube.com/channel/UCIUnjA4kKISKN7_aFz3PGyQ");
		}
		if (Menu::Option("WoozyModZ"))
		{
//			system("start ");
			system("start https://www.youtube.com/channel/UCzzBl-HgDJIvzQ7dyELADeg");
		}
		if (Menu::Option("Nibba Cat"))
		{
			system("start https://www.youtube.com/channel/UChbrB5bvQOmL-p2XjU1g17Q");
		}
		if (Menu::Settings::currentOption == 1) { Menu::InfoText("Click to support -> iAmSpace"); }
		if (Menu::Settings::currentOption == 2) { Menu::InfoText("Click to support -> Trixxy"); }
		if (Menu::Settings::currentOption == 3) { Menu::InfoText("Click to support -> Darks"); }
		if (Menu::Settings::currentOption == 4) { Menu::InfoText("Click to support -> Senpai | Lucid"); }
		if (Menu::Settings::currentOption == 5) { Menu::InfoText("Click to support -> aki"); }
		if (Menu::Settings::currentOption == 6) { Menu::InfoText("Click to support -> Crabi"); }
		if (Menu::Settings::currentOption == 7) { Menu::InfoText("Click to support -> WoozyModZ"); }
		if (Menu::Settings::currentOption == 8) { Menu::InfoText("Click to support -> Nibba Cat"); }
	}
	break;
#pragma region self
	case self:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Player");
		Menu::MenuOption("Outfit", OutfitMenu);
		Menu::MenuOption("Important Options", importantOptions);
		Menu::MenuOption("Movement", movementOptions);
		...
		Menu::Toggle("Off the Radar (Lester)", Features::orbool, [] { Features::OffRadar(Features::orbool); });
		...
		Menu::Toggle("Karma", Features::kkkbool, [] { Features::ktest(); });
		...
		if (Menu::Settings::currentOption == 1) { Menu::InfoText("Customizable Outfit Options"); }
		if (Menu::Settings::currentOption == 2) { Menu::InfoText("Quick / Important Options"); }
		if (Menu::Settings::currentOption == 3) { Menu::InfoText("Movement Options (Fast Run, Swim, etc.)"); }
		if (Menu::Settings::currentOption == 4) { Menu::InfoText("Animation Options"); }
		if (Menu::Settings::currentOption == 5) { Menu::InfoText("Change the model of your character"); }
		if (Menu::Settings::currentOption == 6) { Menu::InfoText("Change your walk style"); }
		if (Menu::Settings::currentOption == 7) { Menu::InfoText("PTFX / Particles"); }
	}
	break;
	case walkstyles:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Walk Styles");
	...
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Object Spawner");
		Menu::MenuOption("Props", spwnr_obj);
		Menu::MenuOption("Peds", spwnr_ped);
	}
	break;
	case spwnr_obj:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Props / Objects");
		for (int i = 0; i < ARRAYSIZE(objects1); i++)
		{
			if (Menu::Option(objects1[i]))
			{
				Vector3 pos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1);
				latestObj = SpawnObject(objects1[i], pos);
			}
		}
	}
	break;
	case spwnr_ped:
	{
		...
		for (int i = 0; i < ARRAYSIZE(pedModels); i++)
		{
			if (Menu::Option(pedModels[i]))
			{
				...
				STREAMING::REQUEST_MODEL(model);
				while (!STREAMING::HAS_MODEL_LOADED(model)) WAIT(0);
				latestObj = latestObj = PED::CREATE_PED(0, GAMEPLAY::GET_HASH_KEY(pedModels[i]), pos.x, pos.y, pos.z, 0.f, 1, 1);
			}
		}
	}
	break;
	...
	break;
#pragma endregion	
	case importantOptions: 
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Important Options");
	...
	case movementOptions:
	{
		...
		Menu::Toggle("No-Clip", Features::flyboolfrnoclip, [] { Features::playerflyer(Features::flyboolfrnoclip); });
		//Walk Clip
		//Walk on Water
		//Walk on Air
		Menu::Toggle("Fast Run", Features::fastRun, [] { Features::runFast(Features::fastRun); });
		Menu::Toggle("Slide Run", Features::isSlideRun, [] { Features::slideRun(Features::isSlideRun); });
		Menu::Toggle("Fast Swim", Features::BackstrokePro, [] { Features::BackstrokeProVoid(); });
		Menu::Toggle("Punch", Features::Flex, [] { Features::flex(Features::Flex); });
		Menu::Toggle("Modded Passive Mode", Features::pussy1[Features::Online::selectedPlayer]);
		Menu::Toggle("Ultrajump", Features::ultrajump, [] { Features::ULTRAJUMP(Features::ultrajump); });
		Menu::Toggle("Superman", Features::supermanOn, [] { Features::Superman(Features::supermanOn); });
		Menu::Toggle("Super Jump", Features::Superjump, [] { Features::superjump(Features::Superjump); });
		Menu::Toggle("Slow Motion", Features::SlowMoBool, [] { Features::SlowMo(Features::SlowMoBool); });
		Menu::Toggle("Time Lapse", Features::FastMoBool, [] { Features::FastMo(Features::FastMoBool); });
		Menu::Toggle("Explosive Mode", Features::explodeLoopBool[Features::Online::selectedPlayer], [] {});
		//??????????????
	}
	break;
#pragma region onlineplayer
	...
	case onlinemenu_selected:
	{
		...
		Menu::Title(Menu::Settings::menuname);
		Menu::Break(TheBreakSayingPlayer.c_str());
	...
	case dropmenyoo:
	{
		...
		Menu::Toggle("Fakedrop", Features::customFakedrop);
	...
	case nice:
	{
		...
		if (Menu::Option("Blacklist")) { Features::addToBlacklist(Features::Online::selectedPlayer); HypnoScript::getFileContent("C:/Hypno/Metadata/Friends/blacklist.txt", Features::blacklist_names); }
	}
	break;
	case grief:
	{
		...
		if (Menu::Option("Hostile Buzzard")) {
	...
	case remote:
	{
		...
		Menu::Toggle("Disable CEO", Features::disableCEObool[Features::Online::selectedPlayer], [] { Features::disableCEO(Features::disableCEObool[Features::Online::selectedPlayer]); });
		...
		if (Menu::Option("Rotate Cam forward")) {
...
#pragma region aioplayer
	...
	case aioplist:
	{
		...
		Menu::MenuOption("Lobby Time", lobbytime);
	...
	case espmenu:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("ESP Menu");
		if (Menu::Option("Quick enable All")) 
		{ 
			Features::ESP::esp2b = true;
			Features::ESP::esper = true;
			Features::ESP::NameESP = true;
		}
		if (Menu::Option("Quick disable All"))
		{
			Features::ESP::esp2b = false;
			Features::ESP::esper = false;
			Features::ESP::NameESP = false;
		}
	...
	case remoteall:
	{
		...
		if (Menu::Option("Teleport to Eclipse Tower")) {
			for (int i = 0; i < 33; i++) {
				if (Features::exclude(i)) {
...
#pragma region vehicle
	...
	case vehicles:
	{
		...
		if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false))
		{
			...
			Menu::IntToggle("Horn Boost", Features::boostbool, [] { Features::carboost(Features::boostbool); }, Features::hornboostInput, 1, 100);
			if (Features::boostbool) {
			...
			Menu::Toggle("Invisible Vehicle", Features::carinvisbool, [] { Features::carinvis(Features::carinvisbool); });
			Menu::Toggle("Stick to Ground", Features::stickToGround, [] { Features::vehStuckToGround(); });
			Menu::Toggle("Bomber Mode", Features::bomber);
			Menu::MenuOption("Cargobob Mods", cargobobmods);
			Menu::MenuOption("Speedometer", speedometa);
			Menu::MenuOption("Vehicle Weapons", vehicleweapons);
			Menu::MenuOption("Vehicle Color", customcarcolor);
		}
		else {
			if (Menu::Option("~m~Vehicle Godmode"))      { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Fix Vehicle"))          { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Max Vehicle"))          { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Rainbow Color"))        { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Horn Boost"))           { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Vehicle Jump"))         { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Drive on Water"))       { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Invisible Vehicle"))    { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Stick to Ground"))      { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Bomber Mode"))          { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Cargobob Mods"))        { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Speedometer"))          { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Vehicle Weapons"))      { Features::notify("~r~You need to be in a vehicle first!"); }
			if (Menu::Option("~m~Vehicle Color"))        { Features::notify("~r~You need to be in a vehicle first!"); }
		}
	}
	break;
	case speedometa:
	{
		...
		if (Features::Speedometer) {
			Menu::Toggle("On Numberplate", Features::numberplateSp);
			Menu::MenuOption("Select Unit", unit);
		}
		else {
			if (Menu::Option("~m~On Numberplate"))   { Features::notify("~r~You need to enable Speedometer first!"); }
			if (Menu::Option("~m~Select Unit"))      { Features::notify("~r~You need to enable Speedometer first!"); }
		}
	}
	break;
	case unit:
...
#pragma region weapon
	...
	case weapons:
	{
		...
		Menu::MenuOption("Remote Gun", remote_gun);
		Menu::MenuOption("Kill Spoofer", kill_spoofer);
		if (Menu::Option("Give All Weapons")) {
		...
		Menu::Toggle("Triggerbot", Features::Triggerbot, [] { Features::TriggerBot(Features::Triggerbot); });
		...
		Menu::Toggle("Ped swap Gun", Features::pedswap);
		Menu::Toggle("Missile Gun", Features::VALKYIREROCKET, [] { Features::valkyirerocket(Features::VALKYIREROCKET); });
...
#pragma region spawner
	...
	case spawner:
	{
		...
		if (Menu::Option("Input Vehicle"))
		{
			...
			if (STREAMING::IS_MODEL_VALID(model))
			{
				...
				Vehicle veh = Hooking::create_vehicle(model, ourCoords.x + xVector, ourCoords.y + yVector, ourCoords.z, heading, true, true);
				...
				if (Features::spawnmaxed)
				{
					...
					VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(veh, "Hypno V2");
				}
	...
	case Supers:
		...
		Features::drawCar(Menu::Settings::currentOption, Super1[Menu::Settings::currentOption - 1]);
		break;
	case Sports:
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Sport");
	...
	case Ulti:
	{
		...
		if (Menu::Option("Trolley")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hash2))
			{
				...
				{
					int Vehicle = Hooking::create_vehicle(hash, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
		...
		if (Menu::Option("6x6 War RC")) {
			...
			{
				int Object1 = Hooking::create_vehicle(hash, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object2 = Hooking::create_vehicle(hash3, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object3 = Hooking::create_vehicle(hash4, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object4 = Hooking::create_vehicle(hash5, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object5 = Hooking::create_vehicle(hash6, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				...
				{
					int Vehicle = Hooking::create_vehicle(hash2, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
					if (ENTITY::DOES_ENTITY_EXIST(Vehicle))
					{
						if (Features::spawnincar)
						{
		...
		if (Menu::Option("Speeder Prototype")) {
			...
			{
				int Object = Hooking::create_vehicle(hash, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object1 = Hooking::create_vehicle(hash1, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				if (ENTITY::DOES_ENTITY_EXIST(Object))
				{
					int Vehicle = Hooking::create_vehicle(hash2, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
					if (ENTITY::DOES_ENTITY_EXIST(Vehicle))
		...
		if (Menu::Option("Military Flightsquad")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hash2) && STREAMING::HAS_MODEL_LOADED(hash1) && STREAMING::HAS_MODEL_LOADED(hash3) && STREAMING::HAS_MODEL_LOADED(hash4) && STREAMING::HAS_MODEL_LOADED(hash5) && STREAMING::HAS_MODEL_LOADED(hash6) && STREAMING::HAS_MODEL_LOADED(hash7) && STREAMING::HAS_MODEL_LOADED(hash8))
			{
				...
				int Object4 = Hooking::create_vehicle(hash5, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object5 = Hooking::create_vehicle(hash6, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object6 = Hooking::create_vehicle(hash7, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object7 = Hooking::create_vehicle(hash8, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				if (ENTITY::DOES_ENTITY_EXIST(Object))
				{
					int Vehicle = Hooking::create_vehicle(hash2, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
					if (ENTITY::DOES_ENTITY_EXIST(Vehicle))
		...
		if (Menu::Option("Bike Robot")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hash2) && STREAMING::HAS_MODEL_LOADED(hash1) && STREAMING::HAS_MODEL_LOADED(hash3) && STREAMING::HAS_MODEL_LOADED(hash4) && STREAMING::HAS_MODEL_LOADED(hash5) && STREAMING::HAS_MODEL_LOADED(hash6) && STREAMING::HAS_MODEL_LOADED(hash7) && STREAMING::HAS_MODEL_LOADED(hash8) && STREAMING::HAS_MODEL_LOADED(hash9) && STREAMING::HAS_MODEL_LOADED(hash10) && STREAMING::HAS_MODEL_LOADED(hash11))
			{
				...
				int Object6 = Hooking::create_vehicle(hash7, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object7 = Hooking::create_vehicle(hash8, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object8 = Hooking::create_vehicle(hash9, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object9 = Hooking::create_vehicle(hash10, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object10 = Hooking::create_vehicle(hash11, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				int Object11 = Hooking::create_vehicle(hash12, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
				if (ENTITY::DOES_ENTITY_EXIST(Object))
				{
					int Vehicle = Hooking::create_vehicle(hash2, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
					if (ENTITY::DOES_ENTITY_EXIST(Vehicle))
		...
		if (Menu::Option("Modded T20")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash_T20) && STREAMING::HAS_MODEL_LOADED(hash2_EMP) && STREAMING::HAS_MODEL_LOADED(hash3_MiniGun) && STREAMING::HAS_MODEL_LOADED(hash4_Wheel))
			{
				...
				{
					int Vehicle = Hooking::create_vehicle(hash_T20, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
		...
		if (Menu::Option("Boat And Trailer")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hash2))
			{
				int Vehicle = Hooking::create_vehicle(hash, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Me), 1, 0);
				int Object = Hooking::create_vehicle(hash2, null.x, null.y, null.z, ENTITY::GET_ENTITY_HEADING(Me), 1, 0);
		...
		if (Menu::Option("JetSki And Trailer")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hash2) && STREAMING::HAS_MODEL_LOADED(hash3))
			{
				int Vehicle = Hooking::create_vehicle(hash, MyCoords.x, MyCoords.y, MyCoords.z, ENTITY::GET_ENTITY_HEADING(Me), 1, 0);
				int Object = Hooking::create_vehicle(hash2, null.x, null.y, null.z, ENTITY::GET_ENTITY_HEADING(Me), 1, 0);
				int Object1 = Hooking::create_vehicle(hash2, null.x, null.y, null.z, ENTITY::GET_ENTITY_HEADING(Me), 1, 0);
		...
		if (Menu::Option("Panto/HotKnife")) {
			...
			if (STREAMING::HAS_MODEL_LOADED(hash) && STREAMING::HAS_MODEL_LOADED(hashSides) && STREAMING::HAS_MODEL_LOADED(hashWindow) && STREAMING::HAS_MODEL_LOADED(hash2_EMP) && STREAMING::HAS_MODEL_LOADED(hash3_MiniGun) && STREAMING::HAS_MODEL_LOADED(hash2) && STREAMING::HAS_MODEL_LOADED(hash4_Wheel))
			{
				...
				int RRWheel1 = OBJECT::CREATE_OBJECT(hash4_Wheel, null5.x, null5.y, null5.z, 1, 0, 1);
				int Object = Hooking::create_vehicle(hash2, null.x, null.y, null.z, ENTITY::GET_ENTITY_HEADING(Handle), 1, 0);
	...
	case dlc_main:
	{
		...
		if (!versionnumber) {
			...
			Menu::MenuOption(dlcnames2[29], dlc_casinoheist);
			Menu::MenuOption(dlcnames2[30], dlc_summerspecial2020);
		}
		else {
			Menu::MenuOption(dlcnames2[0], dlc_beachbum);
			Menu::MenuOption(dlcnames2[1], dlc_valentine1);
		...
		break;
	case dlc_casinoheist:
		Menu::Title(Menu::Settings::menuname);
		Menu::Break(dlcnames[29]);
		for (auto car : CasinoHeist) { Menu::Option(Features::vehicleNames ? VEHICLE::GET_DISPLAY_NAME_FROM_VEHICLE_MODEL($(car)) : car, [car] {Features::spawn_vehicle(car); }); }
		break;
	case dlc_summerspecial2020:
		Menu::Title(Menu::Settings::menuname);
		Menu::Break(dlcnames[30]);
		for (auto car : arenawar) { Menu::Option(Features::vehicleNames ? VEHICLE::GET_DISPLAY_NAME_FROM_VEHICLE_MODEL($(car)) : car, [car] {Features::spawn_vehicle(car); }); }
		break;
	case dlc_beachbum:
		Menu::Title(Menu::Settings::menuname);
...
#pragma region world
	...
	case blacklist_:
	{
		...
		if (loadBlacklist) {
			std::string spath = "C:/Hypno/Metadata/Friends/blacklist_cfg.ini";
			...
			HypnoScript::getFileContent("C:/Hypno/Metadata/Friends/blacklist.txt", Features::blacklist_names);
	...
	case world:
	{
		...
		Menu::Int("Max Players", Features::playerCount, 1, 30);
		Menu::Toggle("Set Max Players", Features::maxPlayers);
	}
	break;
	case modelswap_1:
	{
...
#pragma region misc
	...
	case misc:
	{
		...
		Menu::Toggle("Rockstar Dev Mode", Hooking::rdev);
...
#pragma region recover
	...
	case recoy:
	{
		...
		Menu::MenuOption("Stats Editor", extendedstat);
		Menu::MenuOption("Unlock Options", r_unlocks);
		Menu::MenuOption("~b~Rank ~w~Options", r_levels);
		Menu::MenuOption("Money", customstealth);
		Menu::MenuOption("Bunker Research", bunkerresearch);
		Menu::MenuOption("Spoof Stats", spoofer);
	}
	break;
	case spoofer:
	{
	...
	case r_unlocks:
	{
		...
		if (Menu::Option("Unlock office money")) {
			STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_LIFETIME_BUY_COMPLETE"), 1223, 0);
			STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_LIFETIME_BUY_UNDERTAKEN"), 1223, 0);
			STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_LIFETIME_SELL_COMPLETE"), 434, 0);
			STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_LIFETIME_SELL_UNDERTAKEN"), 434, 0);
...
#pragma region Settings Menu
	case settings:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Settings");
		Menu::MenuOption("Menu UI Options", control_settings);
		Menu::MenuOption("Menu Colors", settingskek);
		Menu::MenuOption("Theme Loader", themeloader);
		Menu::MenuOption("Other Settings", othersettingsforu);
		Menu::MenuOption("Involved People Credits", credits);
		Menu::Toggle("Dump Events", Hooking::logger);
	}
	break;
	case othersettingsforu:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Other Options");
		Menu::MenuOption("Info", settings_sysinfo);
		Menu::MenuOption("Toggle Icons", menutoggles);
		Menu::Toggle("Show Instructions", Features::show_tips);
		if (Menu::Option("Save Theme")) {
			Features::saveToINI();
			INI::WriteBool("C:/Hypno/Metadata/Configuration/config.ini", display_when_open, "display", "close");
			INI::WriteBool("C:/Hypno/Metadata/Configuration/config.ini", reportdisplay, "display", "d_report");
			INI::WriteBool("C:/Hypno/Metadata/Configuration/config.ini", ipdisplay, "display", "d_ip");
			INI::WriteBool("C:/Hypno/Metadata/Configuration/config.ini", Menu::Settings::selfInfo, "display", "d_info");
		}
		if (Menu::Option("Load Theme")) {
			menyooX = INI::GetFloat("C:/Hypno/Metadata/Configuration/config.ini", "theme", "menux") * 100;
			smooth_scroll_kek = INI::GetFloat("C:/Hypno/Metadata/Configuration/config.ini", "delay", "smooth") * 1000;
			display_when_open = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "close");
			reportdisplay = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "d_report");
			ipdisplay = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "d_ip");
			Menu::Settings::selfInfo = INI::GetBool("C:/Hypno/Metadata/Configuration/config.ini", "display", "d_info");
			Features::loadFromINI();
		}
		Menu::Toggle("Rainbow Menu", Menu::RainbowBool, [] { Menu::RainbowMenu(Menu::RainbowBool);  });
		Menu::Int("Menu X", menyooX, 15, 87);
		Menu::Int("Menu Y", menyooY, 0, 30);
		Menu::Int("Menu Width", widthkek, 11, 50);
		Menu::Int("Font", Menu::Settings::optionText.f, 0, 7);
		Menu::Toggle("Custom Sprite Testing", Menu::Settings::customspritetest, [] { Menu::RainbowMenu(Menu::Settings::customspritetest);  });
		if (Menu::Settings::customspritetest) {
			Menu::Int("SpriteX", sprtx, 1, 100);
			Menu::Int("SpriteY", sprty, 1, 100);
			Menu::Int("SpriteWidth", sprtw, 1, 100);
			Menu::Int("SpriteHeight", sprth, 1, 100);
			Menu::Int("SpriteR", sprtr, 1, 255);
			Menu::Int("SpriteG", sprtg, 1, 255);
			Menu::Int("SpriteB", sprtb, 1, 255);
			Menu::Int("SpriteA", sprta, 1, 255);
		}
		Menu::Settings::CustomSpriteX = (float)sprtx;
		Menu::Settings::CustomSpriteY = (float)sprty;
		Menu::Settings::CustomSpriteW = (float)sprtw;
		Menu::Settings::CustomSpriteH = (float)sprth;
		Menu::Settings::CustomSpriteCR = (float)sprtr;
		Menu::Settings::CustomSpriteCG = (float)sprtg;
		Menu::Settings::CustomSpriteCB = (float)sprtb;
		Menu::Settings::CustomSpriteCC = (float)sprta;
		Menu::Settings::menuWidth = (float)widthkek / 100;
		Menu::Settings::scrollbarWidthIndex = (float)(Menu::Settings::menuWidth / 2) + 0.015f;
	...
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Debug Options");
		if (Menu::Option("Set Open Key")) {
			Menu::setOpenKey();
		}
		Menu::Toggle("Banner Glare", Menu::Settings::showGlobe);
		if (Menu::Settings::enable_scrollbar = false)
		{
			Menu::Toggle("~g~Enable Scrollbar", Menu::Settings::enable_scrollbar);
		}
		else if (Menu::Settings::enable_scrollbar = true)
		{
			Menu::Toggle("~r~Disable Scrollbar", Menu::Settings::enable_scrollbar);
		}
		Menu::Toggle("Smooth Scroll", Menu::Settings::enable_smooth);
	...
	case settings_sysinfo:
	{
		...
		std::string RedColor = "~r~";
		std::string GrnColor = "~g~";
		std::string PN = RedColor + nigNog;
		std::string SC = GrnColor + nogNig;
		std::string GV = GrnColor + nibNob;

		//		Menu::Option(fucker.c_str());
		Menu::LROption("Player name: ", PN.c_str());
		Menu::LROption("SC Account: ", SC.c_str());
		Menu::LROption("Game Version: ", GV.c_str());
		Menu::LROption("Menu Version: ", "~g~BETA ~s~| ~r~Build ~s~3248 ");
		// Game Type done by aki#9001
		if (GetModuleHandleA("steam_api64.dll")) { Menu::LROption("Game Type: ", "~bold~~italic~~b~Steam"); }
		else if (GetModuleHandleA("EOSSDK-Win64-Shipping.dll")) { Menu::LROption("Game Type: ", "~bold~~italic~Epic Games"); }
		else { Menu::LROption("Game Type: ", "~bold~~italic~~o~Rockstar"); }
		Hooking::found_idp ? Menu::LROption("IDP Hook: ", "~g~Active") : Menu::LROption("IDP Hook: ", "~r~Inactive");
		Hooking::found_dce ? Menu::LROption("DCE Hook: ", "~g~Active") : Menu::LROption("DCE Hook: ", "~r~Inactive");
		Hooking::found_ssi ? Menu::LROption("SSI Hook: ", "~g~Active") : Menu::LROption("SSI Hook: ", "~r~Inactive");
		Hooking::found_sge ? Menu::LROption("SGE Hook: ", "~g~Active") : Menu::LROption("SGE Hook: ", "~r~Inactive");
		Hooking::found_gcm ? Menu::LROption("GCT Hook: ", "~g~Active") : Menu::LROption("GCM Hook: ", "~r~Inactive");
		Hooking::found_glt ? Menu::LROption("GLT Hook: ", "~g~Active") : Menu::LROption("GLT Hook: ", "~r~Inactive");
		Hooking::found_gsh ? Menu::LROption("GSH Hook: ", "~g~Active") : Menu::LROption("GSH Hook: ", "~r~Inactive");
		Hooking::found_ged ? Menu::LROption("GED Hook: ", "~g~Active") : Menu::LROption("GED Hook: ", "~r~Inactive");
		Hooking::found_nts ? Menu::LROption("NTV Hook: ", "~g~Active") : Menu::LROption("NTV Hook: ", "~r~Inactive");
		Hooking::found_prw ? Menu::LROption("PRW Hook: ", "~g~Active") : Menu::LROption("PRW Hook: ", "~r~Inactive");
	}
	break;
	case settingskek:
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Menu Colors");
		if (Menu::Int("Scroller ~r~Red", Menu::Settings::scroller.r, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.r = NumberKeyboard(); } }
		if (Menu::Int("Scroller ~g~Green", Menu::Settings::scroller.g, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.g = NumberKeyboard(); } }
		if (Menu::Int("Scroller ~b~Blue", Menu::Settings::scroller.b, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.b = NumberKeyboard(); } }
		if (Menu::Int("Scroller Alpha", Menu::Settings::scroller.a, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.a = NumberKeyboard(); } }
		if (Menu::Int("Background ~r~Red", Menu::Settings::optionRect.r, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::optionRect.r = NumberKeyboard(); } }
		if (Menu::Int("Background ~g~Green", Menu::Settings::optionRect.g, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::optionRect.g = NumberKeyboard(); } }
		if (Menu::Int("Background ~b~Blue", Menu::Settings::optionRect.b, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::optionRect.b = NumberKeyboard(); } }
		if (Menu::Int("Background Alpha", Menu::Settings::optionRect.a, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::optionRect.a = NumberKeyboard(); } }
		if (Menu::Int("Subtitle + End ~r~Red", Menu::Settings::breakrect.r, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::breakrect.r = NumberKeyboard(); } }
		if (Menu::Int("Subtitle + End ~g~Green", Menu::Settings::breakrect.g, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::breakrect.g = NumberKeyboard(); } }
		if (Menu::Int("Subtitle + End ~b~Blue", Menu::Settings::breakrect.b, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::breakrect.b = NumberKeyboard(); } }
		if (Menu::Int("Subtitle + End Alpha", Menu::Settings::breakrect.a, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::breakrect.a = NumberKeyboard(); } }
		if (Menu::Int("Arrow ~r~Red", Menu::Settings::arrow.r, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.r = NumberKeyboard(); } }
		if (Menu::Int("Arrow ~g~Green", Menu::Settings::arrow.g, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.g = NumberKeyboard(); } }
		if (Menu::Int("Arrow ~b~Blue", Menu::Settings::arrow.b, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.b = NumberKeyboard(); } }
		if (Menu::Int("Arrow Alpha", Menu::Settings::arrow.a, 0, 255)) { if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) { Menu::Settings::scroller.a = NumberKeyboard(); } }
/*		Menu::MenuOption("Selection Box", settingsscroller);
	...
	case menutoggles:
	{
		...
		if (Menu::Option("Tick (White)")) {
			Menu::Settings::toggleYTD = "commonmenu";
			Menu::Settings::toggleON  = "shop_box_tick";
			Menu::Settings::toggleOFF = "shop_box_blank";
		}
		if (Menu::Option("Tick (Black)")) {
			Menu::Settings::toggleYTD = "commonmenu";
			Menu::Settings::toggleON  = "shop_box_tickb";
			Menu::Settings::toggleOFF = "shop_box_blankb";
		}
		if (Menu::Option("Ammo")) {
			Menu::Settings::toggleYTD = "commonmenu";
	...
	{
		Menu::Title(Menu::Settings::menuname);
		Menu::Break("Themes + Customs");
		Menu::MenuOption("Custom Header", i_like_seys);
		if (Menu::Option("Hypno's Default Theme")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "Hypno1";
			Menu::Settings::scroller = { 109, 10, 0, 255 };
			Menu::Settings::breakrect = { 0, 0, 0, 255 };
			Menu::Settings::optionRect = { 0, 0, 0, 170 };
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			widthkek = 26;
			widthkek2 = 26;
			Menu::Settings::menuWidth = 0.26f;
			Menu::Settings::showGlobe = false;
			Menu::Settings::enable_smooth = true;
			Menu::Settings::lightning = false;
			Menu::Settings::c_enable = false;
			Menu::Settings::c_animation = false;
			Menu::Settings::evolve = false;
			Menu::Settings::evolve2 = false;
			Menu::Settings::phantom = false;
			Menu::Settings::city = false;
			overtheme = false;
		}
		if (Menu::Option("Hypno's New Beta Theme")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "IMG1";
			Menu::Settings::scroller = { 192, 0, 255, 255 };
			Menu::Settings::breakrect = { 0, 0, 0, 255 };
			Menu::Settings::optionRect = { 0, 0, 0, 170 };
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::menuWidth = 0.26f;
		...
		Menu::MenuBreak("~r~[ ~c~Headers only, not themes. ~r~]");
		if (Menu::Option("Hypno Default")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "Hypno1";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
		...
		if (Menu::Option("[~g~NEW~w~] ~r~J~w~a~r~p~w~a~r~n~w~e~r~s~w~e ~r~Hypno")) {
			Menu::Settings::menuname = "";
			Menu::Settings::c_pict = "HypnoCustomDefaultHQ";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::c_enable = true;
			Menu::Settings::lightning = false;
			Menu::Settings::evolve = false;
			Menu::Settings::c_animation = false;
			Menu::Settings::evolve2 = false;
		...
		if (Menu::Option("Hypno Default 2")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "Hypno2";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::lightning = false;
			Menu::Settings::c_enable = false;
		...
		if (Menu::Option("Hypno Default 3")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "Hypno3";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::lightning = false;
			Menu::Settings::c_enable = false;
		...
		if (Menu::Option("Hypno Default 4")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "Hypno4";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::lightning = false;
			Menu::Settings::c_enable = false;
		...
		Menu::MenuBreak("~r~[ ~c~Special ~r~]");
		if (Menu::GlowOption("Over")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "OverIMG";
			Menu::Settings::scroller = { 0, 102, 255, 255 };
			Menu::Settings::breakrect = { 24, 27, 46, 255 };
			Menu::Settings::optionRect = { 33, 37, 63, 255 };
			Menu::Settings::scrollertop = { 24, 27, 46, 255 };
			Menu::Settings::menuWidth = 0.22f;
			widthkek = 22;
			widthkek2 = 22;
			Menu::Settings::enable_smooth = false;
			Menu::Settings::showGlobe = false;
			Menu::Settings::c_enable = false;
			overtheme = true;
			Menu::Settings::lightning = false;
		...
		if (Menu::GlowOption("Evolve")) {
			Menu::Settings::menuname = "";
			Menu::Settings::ytdtheme = "evolveanim1";
			Menu::Settings::showGlobe = false;
			Menu::Settings::scrollertop = { 24, 27, 46, 0 };
			Menu::Settings::lightning = false;
			Menu::Settings::c_enable = false;
			Menu::Settings::evolve = false;
			Menu::Settings::evolve2 = true;
			Menu::Settings::phantom = false;
			Menu::Settings::city = false;
			overtheme = false;
		}
...
void ScriptMain() {
	while (true) {
		...
		std::string WelcomeNotification = "Welcome ~r~" + SocialClubNick + " ~w~to ~g~HypnoV2~w~! Press ~r~* ~w~or ~r~F5 ~w~to dismiss.";
		
		if (bannernoti)
			notifybanner(WelcomeNotification.c_str());
...
void stupid()
{
	...
	{
		handle = GRAPHICS::REQUEST_SCALEFORM_MOVIE("instructional_buttons");
		buttonboi.CallFunction((char*)"SET_DATA_SLOT_EMPTY");

		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD("SET_DATA_SLOT", 0, 't_F', 't_F');
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD("SET_DATA_SLOT", 1, 't_G1', 't_G1');
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD("SET_DATA_SLOT", 2, 't_M', 't_M');
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD("SET_DATA_SLOT", 3, 't_<a', 't_<a');
...
void DisplayScript::Tick() {
	...
	std::string PlayerNameSelected1(SOCIALCLUB::_SC_GET_NICKNAME());
	std::string InstrucitonsWithNickname = "SC Username: " + PlayerNameSelected1;
	if (Features::show_tips) {
		buttons->setup();
		buttons->add((char*)InstrucitonsWithNickname.c_str(), 53);
		buttons->add((char*)"Open", 134);
//		sc.CallFunction('SET_DATA_SLOT', 0, 'entertest', 'w_ENTER');
		buttons->add((char*)"Open", 174);
		buttons->add((char*)"Back", 136);
		buttons->add((char*)"Up", 144);
		buttons->add((char*)"Down", 138);
		buttons->add((char*)"Left", 140);
		buttons->add((char*)"Right", 142);
		buttons->add((char*)"Enter", 135);
		   if (Cursor::cursorOn == false) { buttons->add((char*)"Cursor (Off)", 181); }
		else if (Cursor::cursorOn == true) { buttons->add((char*)"Cursor (On)", 181); }
		buttons->draw();
	}
}
